...
class ENV:
    ...
    def render(self, episodes, n_finished):
        screen.fill((0, 0, 0))
        ...
        self.render_ui(episodes, n_finished)
        pygame.display.update()

    def render_ui(self, epsilons, n_finished):
        font = pygame.font.SysFont("Comic Sans MS", 20)
        label = font.render(f"Epsilons: {epsilons} | finished: {n_finished}", 1, (230,230,255))
...
epsilon = 5 # exploration (random action) factor
min_epsilon = 0.1
epsilon_decay = 0.99
num_episodes = 10000
max_steps = 20
...
while True:
    for i in range(num_episodes):
        #print(f"Episode: {i} | end state {Env.state}")
        is_done = False
        state = Env.reset()
        for j in range(max_steps):

            # Rendering
            Env.render(i,n_finished)
