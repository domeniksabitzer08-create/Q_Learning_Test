...
class ENV:
    ...
    def step(self, action) -> (int, float, bool):
        ...
        self.state = next_state
        return (next_state.y * cell_count_x + next_state.x), reward, is_done
    ...
    def render(self, episodes, n_finished, epsilon, step):
        screen.fill((0, 0, 0))
        ...
        self.render_ui(episodes, n_finished, epsilon, step)
        pygame.display.update()

    def render_ui(self, episodes, n_finished, epsilon, step):
        font = pygame.font.SysFont("Comic Sans MS", 20)
        acc = n_finished/(episodes+1) *100
        label = font.render(f"Episodes: {episodes} | finished: {n_finished} | epsilon: {epsilon:.2f}", 1, (230,230,255))
        label2 = font.render(f"accuracy: {acc:.2f}% | steps: {step}", 1, (230,230,255))
        self.screen.blit(label, (50, 330+20))
        self.screen.blit(label2, (50, 350+20))
...
# Grid setup
cell_count_x, cell_count_y = 6, 5
reward_pos = Vector2(5,4) # in grid coordinates
# Maze #
empty_maze = blocks = []

easy_maze = [Vector2(2,2), Vector2(2,1)] # start pos: 0,2 | reward_pos: 4, 2 | cell count 5,3
mid_maze = [Vector2(2,0), Vector2(2,1), Vector2(2,2), Vector2(2,3), Vector2(4,1), Vector2(4,2), Vector2(4,3), Vector2(4,4)] # start pos: 0,0 | reward_pos: 5, 4 | cell count 6,5
...
for episode in tqdm(range(num_episodes)):
    ...
    for j in range(max_steps):
        ...
        # Rendering
        #Env.render(episode, train_n_finished, epsilon, j)
        action = choose_action(state)
        next_state, reward, is_done = Env.step(action)
        old_value = q_table[state,action]
        next_max = np.max(q_table[next_state])

        q_table[state,action] = old_value + lr *(reward + gamma * next_max ) - old_value
...
acc = (train_n_finished / num_episodes) * 100
print(f"\n accuracy: {acc:.2f}% | finished: {train_n_finished} | episodes: {num_episodes}")
...
for episode in range(test_episodes):
    ...
    for step in range(max_steps):
        # Rendering
        Env.render(episode, test_n_finished, epsilon, step)
        sleep(0.1)
        action = choose_action(state)
        next_state, reward, is_done = Env.step(action)
        state = next_state
